...
1. Set `ArcDemo.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../ArcDemo.Infrastructure/ArcDemo.Infrastructure.csproj -s ArcDemo.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../ArcDemo.Infrastructure/ArcDemo.Infrastructure.csproj -s ArcDemo.Web.csproj
```
